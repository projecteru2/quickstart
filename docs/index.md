...
```
   ...
   ┌──────────┼───────────────┬──────────────────────┐
   │          │               │                      │
[etcd]     [core]     [node_containerd]        [node_process]
   │          │               │                      │
 etcd     eru-core         containerd             systemd
 :2379       │  │             │                      │
   ▲         │  └─ ssh ──────►│◄──────── ssh ────────┘
   └─────────┘             eru-agent              eru-agent
...
`containerd://` for containers, `process://` for bundles that run as transient systemd units.
